...
router.post('/stock-in', requirePermission('stockIn'), async (req, res) => {
  try {
    const { productId, warehouseId, supplierId, quantity, rack, bin } = req.body;
...
router.post('/stock-out', requirePermission('stockOut'), async (req, res) => {
  try {
    const { productId, warehouseId, quantity, customer, notes, reference } = req.body;
    ...
    const ref = reference || genRef('OUT');
    const movement = await Movement.create({
      product:       productId,
      type:          'OUT',
      quantity,
