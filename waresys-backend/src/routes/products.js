...
router.get('/', async (req, res) => {
  try {
    ...
    const result = products.map(p => {
      ...
      return { id: p._id, _id: p._id, name: p.name, sku: p.sku, category: p.category, unit: p.unit, totalQty: qty, status };
    });
...
router.put('/:id', requirePermission('addProduct'), async (req, res) => {
  try {
    ...
    // Check for duplicate SKU on a different product
    if (sku) {
      const dup = await Product.findOne({ sku: sku.trim(), _id: { $ne: req.params.id } });
      if (dup)
        return res.status(409).json({ success: false, message: `SKU "${sku}" already exists on another product.` });
    }
